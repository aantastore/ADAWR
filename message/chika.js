	...
	switch (command) {
            ...
            case prefix+'menu': case prefix+'help':{
                ...
                if (isGroup) {
                    ...
                    sendButton('location', from, `මචන් හායි ${pushname} 👋, me *${botName}*\n\nමෙම bot Beta *Multi-Device* Whatsapp වේ. ඔබ යම් ආකාරයක දෝෂයක් හෝ දෝෂයක් සොයා ගන්නේ නම්, කරුණාකර පළමුව තේරුම් ගන්න, හරිද?😖, අවශ්‍ය හෝ හදිසි නම් අයිතිකරු වාර්තා කරන්න🙏`, buttons)
               } else {
            ...
            case prefix+'toimg': case prefix+'stickertoimg': case prefix+'stoimg': case prefix+'stikertoimg': 
				if (isQuotedSticker) {
			    	...
			    	if (msg.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated) {
                        await textImg(ind.wait())
                        await reply('සමාවන්න, gif සහාය නොදක්වයි')
					} else {
                        await textImg(ind.wait())
